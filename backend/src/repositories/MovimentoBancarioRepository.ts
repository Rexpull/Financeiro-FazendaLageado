...
export class MovimentoBancarioRepository {
	...
	async getAll(): Promise<MovimentoBancario[]> {
		const { results } = await this.db.prepare(`
			SELECT id, dtMovimento, historico, idPlanoContas, idContaCorrente, valor, saldo, ideagro, numeroDocumento, descricao, transfOrigem, transfDestino, identificadorOfx, criadoEm, atualizadoEm
	...
	async create(movimento: MovimentoBancario): Promise<number> {
		...
		const { meta } = await this.db.prepare(`
			INSERT INTO MovimentoBancario (dtMovimento, historico, idPlanoContas, idContaCorrente, valor, saldo, ideagro, numeroDocumento, descricao, transfOrigem, transfDestino, identificadorOfx, criadoEm, atualizadoEm)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'));
		`).bind(
			dtMovimento, historico, idPlanoContas, idContaCorrente, valor, saldo, ideagro, numeroDocumento, descricao, transfOrigem, transfDestino, identificadorOfx
		).run();
	...
	async update(id: number, movimento: MovimentoBancario): Promise<void> {
		...
		await this.db.prepare(`
			...
			SET dtMovimento = ?, historico = ?, idPlanoContas = ?, idContaCorrente = ?, valor = ?, saldo = ?, ideagro = ?, numeroDocumento = ?, descricao = ?, transfOrigem = ?, transfDestino = ?, identificadorOfx = ?, atualizadoEm = datetime('now')
			WHERE id = ?;
		`).bind(
