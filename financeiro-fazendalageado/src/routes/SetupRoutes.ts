export async function handleRequest(req: Request, DB: D1Database): Promise<Response> {
  ...
  try {
    ...
    if (method === "POST" && pathname === "/api/bancos") {
      ...
      await DB.prepare("INSERT INTO banco (nome, codigo) VALUES (?, ?)").bind(nome, codigo).run();
      console.log("✅ Banco criado:", nome, codigo);
      return new Response(JSON.stringify({ message: "Banco criado com sucesso!" }), { status: 201, headers: corsHeaders });
    }
