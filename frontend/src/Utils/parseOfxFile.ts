...
/** True when transaction fields use explicit closers (Caixa XML-style). */
function ofxHasSelfClosedTransactionFields(compact: string): boolean {
	return (
		/<TRNAMT>[^<]+<\/TRNAMT>/i.test(compact) &&
		/<FITID>[^<]+<\/FITID>/i.test(compact)
	);
}
...
function normalizarOfx(ofx: string): string {
	...
	// Bradesco/SGML mix </STMTTRN> with open <TRNAMT> — only skip when amount/FITID are closed
	if (ofxHasSelfClosedTransactionFields(compact)) {
		return compact;
	}
...
function buildIdentificadorImportacao(fitid: string, dtMovimentoISO: string, valor: number, refNum?: string, checkNum?: string): string {
...
export function extractSantanderBoletoPayee(historico: string): string | null {
	...
	const withPayee = text.match(/PAGAMENTO\s+DE\s+BOLETO(?:\s+OUTROS\s+BANCOS)?\s{2,}(.+)$/i);
...
function agruparLista(lista: MovimentoTemporario[], historicoAgrupado: string): MovimentoTemporario {
...
function agruparMovimentosSantander(movimentos: MovimentoTemporario[]): MovimentoTemporario[] {
	...
	Object.entries(porChave).forEach(([chave, lista]) => {
		...
		if (chave.endsWith('|IOF')) {
			saida.push(agruparLista(lista, `IOF - Agrupado (${lista.length} movimentos)`));
		} else {
			const payee = chave.split('|BOLETO|')[1] ?? '';
			saida.push(agruparLista(lista, `Pagamento de Boleto - Agrupado (${lista.length}) - ${payee}`));
		}
	});
...
function agruparDuplicadosPorFITIDMesmoDia(movimentos: MovimentoTemporario[]): MovimentoTemporario[] {
	const grupos: Record<string, MovimentoTemporario[]> = {};
	const chave = (m: MovimentoTemporario) => `${m.identificadorOfx}|${m.dtMovimento.substring(0, 10)}|${m.tipoMovimento}`;
	...
	Object.entries(grupos).forEach(([k, lista]) => {
		...
		const todosRefIgual = lista.every((x) => x.refNum === base.refNum) ?? true;
		const todosCheckIgual = lista.every((x) => x.checkNum === base.checkNum) ?? true;
...
export function parseOFXContent(ofxContent: string): { movimentos: MovimentoBancario[]; totalizadores: TotalizadoresOFX } {
	...
	transactions.forEach((transaction) => {
		...
		const isDepositoBloqueado = /dep[oó]sito bloquead/i.test(name) || /dep[oó]sito bloquead/i.test(memo);
	...
	movimentos.sort((a, b) => new Date(a.dtMovimento).getTime() - new Date(b.dtMovimento).getTime());

	const dtInicialExtrato = movimentos.length ? movimentos[0].dtMovimento : '';
	const dtFinalExtrato = movimentos.length ? movimentos[movimentos.length - 1].dtMovimento : '';

	const totalizadores = calcularTotalizadoresExtrato(movimentos, undefined, dtInicialExtrato, dtFinalExtrato, trnTypeByIdentificadorOfx);
