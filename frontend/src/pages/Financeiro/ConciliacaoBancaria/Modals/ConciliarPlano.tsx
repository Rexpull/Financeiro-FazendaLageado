...
const ConciliaPlanoContasModal: React.FC<ConciliaPlanoContasModalProps> = ({ isOpen, onClose, movimento, planos, handleConcilia }) => {
  ...
  const [formData, setFormData] = useState<any>({
	...
	const filteredPlanos = planosFetch.filter((p) => p.descricao.toLowerCase().includes(searchPlano.toLowerCase())).slice(0, 10);

	// Filtro Pessoa
	const [searchPessoa, setSearchPessoa] = useState('');
	const filteredPessoas = pessoas.filter((p) => p.nome.toLowerCase().includes(searchPessoa.toLowerCase())).slice(0, 10);

	// Filtro Banco
	const [searchBanco, setSearchBanco] = useState('');
	const filteredBancos = bancos.filter((b) => b.nome.toLowerCase().includes(searchBanco.toLowerCase())).slice(0, 10);
				...
				<>
					<div className="grid grid-cols-2 gap-4 mb-4">
						<div>
							...
							<input type="text" value="Financiamentos" readOnly className="w-full border p-2 rounded bg-gray-100" />
						</div>
						<div>
						...
						<div className="col-span-2 flex items-center gap-2">
							<input type="checkbox" checked={parcelado} onChange={() => setParcelado(!parcelado)} />
					...
					{['padrao', 'financiamento', 'transferencia'].map((tipo) => (
						<button
							...
							className={`px-4 flex-1 text-center text-lg py-1 font-semibold ${
								modalidade === tipo ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-700'
							}`}
						>
							{tipo === 'padrao' && 'Padrão'}
